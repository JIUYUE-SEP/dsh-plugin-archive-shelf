...
export function apply(ctx) {
  ...
  const deleteSession = async (sessionId) => {
    ...
    if (sessionDir === undefined) {
      // Nothing on disk: the durable record is already gone. The archive entry
      // stays — dropping it here would un-hide the session for every browser
      // that still lists it, which is how an unopenable sidebar row appears.
      // Only the client's verified forget (after it re-pulled its own list)
      // removes the entry.
      const warnings = await detachFromWorkspaces(sessionId)
      return { ok: true, cleanedOnly: true, removedPath: '', freedBytes: 0, warnings }
    }
  ...
  const sweep = async () => {
    ...
    for (let pass = 0; pass < 3; pass++) {
      ...
      for (const id of pending) {
        ...
        // The archive entry is deliberately left in place: it is what keeps the
        // session out of the sidebar while browsers may still hold it in a
        // stale list. The browser half drops it once its own list stopped
        // carrying the row; until then the record shows as a dangling one.
        await withQueue(ids => ids.filter(queuedId => queuedId !== id))
    ...
    sweeping = sweep()
      .then((result) => {
        if (result.deleted.length > 0) {
          lastSweep = { deleted: result.deleted.length, ids: result.deleted, at: Date.now() }
  ...
  if (timer !== undefined) {
    for (const delay of SWEEP_DELAYS_MS) {
      // Returning the drain promise is deliberate: the scheduled work is a
      // single memoized sweep, so handing it back makes the sweep awaitable
      // (and lets a test await the real completion instead of guessing ticks).
      ctx.effect(() => timer.timeout(() => drainQueue(), delay), `archive-shelf: queue sweep after ${delay}ms`)
    }
  }
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        switch (action) {
          case 'list': {
            // Reading the shelf never acts on the queue. A sweep here would turn
            // "queue this" into "delete this" for any row that happened to be
            // deletable by the time the next list arrived — without the
            // confirmation dialog the delete action owns. The queue is honoured
            // by the boot sweep only, which is exactly what the copy promises.
