window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			/**
			 * Re-pull the browser's own session list. A released session left that
			 * list for good (the host announces its disposal, and the browser drops
			 * the row), so restoring one needs the same re-pull a page reload does.
			 * @returns whether the list baseline was re-pulled.
			 */
			...
			const forgetStale = async (id) => {
				if (!(await refreshSessionList())) return false;
			...
			function ArchiveShelf(props) {
				...
				const restore = async (row) => {
					...
					try {
						...
						if (result !== null && typeof result === "object" && result.ok === true) {
							// Both halves are needed: the list must carry the session again
							// (a released one was dropped from it), and the archive set must
							// stop hiding it. Either one alone leaves the sidebar unchanged.
							const listed = await refreshSessionList();
							const synced = listed && await syncArchiveSet(
